...
class KlaudBot(commands.Bot):
    ...
    Inherits from commands.Bot and holds all shared service instances.
    All cogs receive a reference to the bot and access services via bot.services.
    """
    ...
    async def setup_hook(self) -> None:
        """Called by discord.py after login, before connecting to gateway."""
        ...
        # Initialise AI service
        await self.gemini.initialise()
    ...
    async def on_ready(self) -> None:
        ...
        logger.info(f"AI available: {self.gemini.available}")
    ...
    async def on_guild_join(self, guild: discord.Guild) -> None:
        ...
        # Try to find a system channel to send the welcome message
    ...
    # ─── Utility methods used by cogs ────────────────────────────────────────

    async def assert_licensed(self, interaction: discord.Interaction) -> bool:
        """
        Check if the interaction's guild has a valid license.
        If not, sends the unlicensed message and returns False.
        The calling command should return immediately if this returns False.
        Owner always passes.
        """
    ...
    async def get_mod_log_channel(
        ...
        """Find the mod log channel for a guild by configured name."""
    ...
    async def log_mod_action(
        ...
        """Write a moderation action to the database."""
        try:
    ...
    async def increment_warn_count(self, guild_id: int, user_id: int) -> int:
        """Increment warn count for a user. Returns new count."""
        try:
            if self.db.is_postgres():
    ...
    def track_spam(self, guild_id: int, user_id: int) -> int:
        ...
        Returns the number of messages in the spam window.
        """
        import time
        now = time.monotonic()
    ...
    async def _safe_respond(
        ...
        """Respond to an interaction safely (handles already-responded case)."""
        try:
    ...
    async def _cache_cleanup_loop(self) -> None:
        """Periodically purge stale license cache entries."""
        try:
    ...
    async def _license_expiry_check(self) -> None:
        """Periodically disable expired licenses in the database."""
        try:
            if self.db.is_postgres():
