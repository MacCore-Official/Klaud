...
class AdminAICog(commands.Cog, name="AdminAI"):
    ...
    async def _process(self, message: discord.Message, instruction: str, tier: str) -> None:
        guild = message.guild
        def _names(items, n): return ", ".join(x.name for x in list(items)[:n])
        guild_context = (
            f"Guild: {guild.name} | Current channel: {message.channel.name}\n"
            f"Categories: {_names(guild.categories, 8)}\n"
            f"Text channels: {_names(guild.text_channels, 12)}\n"
            f"Roles: {_names((r for r in guild.roles if not r.is_default()), 12)}\n"
            f"Bot top role: {guild.me.top_role.name}"
        )
    ...
    async def _log_audit(self, guild, user_id, action_type, details, success=True, error=None) -> None:
        try:
            ds = json.dumps(details, ensure_ascii=True, default=str)[:2000] if details else "{}"
